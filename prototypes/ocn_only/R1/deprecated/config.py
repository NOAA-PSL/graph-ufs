...
class OcnTrainer(BaseOcnTrainer):
    ...
    # === Time settings ===
    delta_t_model = "24h"       # model time step
    delta_t_data = "6h"        # time resolution of data
    input_duration = "48h"      # input window = ICs + delta_t
    target_lead_time = "24h"    # forecast lead time for comparison

    # === Normalization statistics paths ===
    _base_path = (
        "/global/homes/n/nagarwal/graph-ufs/prototypes/ocn_only/statistics/"
        f"ocn_only_10L_fineTop/{delta_t_model}"
    )
